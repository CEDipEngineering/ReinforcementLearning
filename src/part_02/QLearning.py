    ...
    def train(self, filename, plotFile, actionsName):
        ...
        for i in range(1, self.episodes+1):
            ...
            while not done:
                ...
                next_max = self.gamma*np.max(self.q_table[next_state]) #np.max(`do maior valor considerando next_state`)
                new_value = self.alpha*(reward + next_max - old_value) #calcula o novo valor
